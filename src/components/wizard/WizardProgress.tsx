// src/components/wizard/WizardProgress.tsx - ADAPTATIVO CON SISTEMA DE TEMAS
...
const WizardProgress: React.FC<WizardProgressProps> = ({ 
  ...
  const getStepIcon = (stepId: string) => {
    ...
    return (
      <div className="p-2 bg-gradient-to-br from-muted to-muted/80 rounded-full shadow-md">
  ...
  const getStepNumber = (index: number) => {
    ...
    return (
      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-muted to-muted/80 flex items-center justify-center text-sm font-medium text-muted-foreground shadow-md">
        {index + 1}
      </div>
    );
  };
  ...
  return (
    <Card className="mb-6 w-full border-0 shadow-lg bg-card">
      <CardContent className="p-4 lg:p-6">
        {/* Progress Bar */}
        <div className="mb-6">
          <div className="flex justify-between text-sm text-muted-foreground mb-2">
            <span className="font-medium">Progreso del Wizard</span>
            <span className="text-primary font-bold">{getCurrentStepIndex() + 1} de {steps.length}</span>
          </div>
        ...
        <div className="mt-6 flex items-center justify-between text-xs">
          <div className="flex items-center space-x-4">
            ...
            <span className="flex items-center bg-blue-100 dark:bg-blue-900/30 px-3 py-1 rounded-full">
              <Circle className="h-3 w-3 text-blue-600 dark:text-blue-400 mr-1" />
              <span className="text-blue-700 dark:text-blue-300 font-medium">1 actual</span>
            </span>
          ...
          <Badge variant="outline" className="border-primary/20 text-primary bg-primary/5 shadow-sm">
            BSE - AUTOMÓVILES
          </Badge>
        </div>
      </CardContent>
