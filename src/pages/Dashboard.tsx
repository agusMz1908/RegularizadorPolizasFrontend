...
const Dashboard: React.FC = () => {
  ...
  // Mock data para compañías y ramos (esto puede venir del backend también)
  ...
  const handleCreatePoliza = () => {
    if (selectedCompania && selectedRamo) {
      ...
      // Aquí se redirigiría a ProcesarPoliza
                    ...
                    {polizasFiltradas.map((poliza) => (
                      <tr key={poliza.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900">
                          {poliza.prima > 0 ? `${poliza.prima.toLocaleString()} ${poliza.moneda}` : '-'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                {polizasFiltradas.length === 0 && filtroPoliza && (
                  <div className="p-8 text-center text-gray-500">
